...
**DoD:** advisors 0 ERROR; espejo `db/proposed/b045_cancellation_policy.sql`; sin tocar RPCs compartidos.
...
**DoD:** advisors 0 ERROR; espejo b046; `verificar_invariantes` NO tocado (el check "Σrefunds ≤ Σpayments" ya lo cubre el guard; si el carril backend quiere el check formal, va como función/entrada aparte).

## C4 — Cancelar con política (**b047**)
...
- **BD/RPCs (b045–b047) son carril backend por el modelo de 2 agentes** — este plan está escrito para entregárselo tal cual; si el fundador decide que este carril lo haga, se sigue el mismo protocolo (aplicar → espejo → hard-test rollback → advisors).

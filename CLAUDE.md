...
3. **Ledger append-only**: cancelaciones/correcciones son asientos nuevos (`payment` tipo `refund`), no updates/deletes. **Desde 2026-07-19 se aplica en la BD, no sólo en la app**: trigger `no_mutar` (BEFORE DELETE OR TRUNCATE) + `REVOKE DELETE,TRUNCATE` sobre `payments`, `receipts`, `receipt_counters`, `system_log`. `bookings` queda fuera a propósito (se actualiza legítimamente). Ver `db/proposed/002_ledger_inmutable.sql`.
   **Desde 2026-08-04 (b051) `payments` tampoco es UPDATE-able por el cliente**: se revocó `UPDATE` a `authenticated`/`anon` y se dropeó la policy `payments_scoped_upd`. Era un hueco real: con GRANT de tabla + policy sin restricción de columnas, cualquier autenticado podía `PATCH /rest/v1/payments` por PostgREST y subir `amount_mxn` (venta "pagada" sin dinero) o borrar `credit_id` (saldo del crédito es derivado ⇒ gastarlo infinitas veces). **Toda escritura sobre `payments` va por RPC** (`register_payment`, `refund_payment`, `refund_payment_partial`, `redeem_credit`, `confirm_online_payment`) o `service_role`. Mismo criterio para `credits` (RPC-only-write). Ver `db/proposed/b051_credits_hardening.sql`.
   **Corolario para cualquier tabla de dinero nueva:** un `GRANT` de tabla + policy que no restrinja columnas ES escritura arbitraria por PostgREST, aunque la app "nunca lo haga". El patrón correcto del repo es tabla RPC-only-write (`profiles` b017, `sales_goals`, `agency_invitations`, `credits` b051). Y si un saldo se **deriva** de una columna, esa columna no puede ser escribible por el cliente.

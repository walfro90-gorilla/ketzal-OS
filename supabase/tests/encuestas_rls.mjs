...
// en el `finally`. No hay que preparar nada ni existe contraseña que guardar.
...
// La "otra agencia" se deriva del catálogo: hardcodear un segundo UUID es otra
// cosa que se pudre cuando cambian los datos.
const otras = await (await fetch(
  `${U}/rest/v1/suppliers?select=id,name&supplier_type=eq.agency&id=neq.${WANDERLUST}&limit=1`,
  { headers: svc() },
)).json()
if (!otras?.[0]?.id) {
  console.error('No hay una segunda agencia con la que probar el cruce de tenants; abortando.')
  process.exit(1)
}
const OTRA = otras[0]
...
const CUENTAS = [
  { etiqueta: `admin de ${OTRA.name} (otra agencia)`, llave: 'otroadmin' },
  { etiqueta: 'agente no-admin de Wanderlust', llave: 'wlagente' },
  { etiqueta: 'viajero del marketplace', llave: 'viajero' },
]
...
try {
  ...
  // Pase lo que pase: las cuentas no sobreviven a la corrida.

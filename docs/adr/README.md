...
| [0053](0053-el-contenido-de-destino-se-edita-en-el-panel.md) | El contenido editorial de cada destino vive en `ketzal.destinos` y se edita en el panel del god admin (b095); la LISTA la sigue mandando el catálogo, con borradores que no llegan a la vitrina y lectura pública solo por RPC | aceptada |
| [0054](0054-el-mapa-es-svg-del-servidor-y-lo-de-fuera-se-lista.md) | El mapa de destinos es SVG del servidor (trazo de Natural Earth, dominio público, ~2 KB, sin JavaScript) con puntos que son enlaces reales; lo que no cae en México se lista aparte, y las coordenadas no dependen de que el texto esté publicado (b096) | aceptada |
| [0055](0055-el-costeo-es-un-plan-no-un-ledger.md) | El costeo de un tour es un plan, no un ledger (no crea CxP ni toca `expenses`); el costo lo ve solo el admin de la agencia y vive en tablas aparte (`supplier_rate_cards`, `service_costings`, b097), nunca en `services`/`add_ons`; el tarifario lo captura la agencia; punto de equilibrio por escaneo | aceptada |
| [0056](0056-quien-vende-es-dueno-de-sus-servicios.md) | Quién vende se sabe por la RELACIÓN (es dueño de `services.supplier_id`), no por `supplier_type`, que es etiqueta de lectura; el subtipo es texto libre y no decide nada. Hueco declarado: la BD aún no lo obliga | aceptada |
| [0057](0057-la-ubicacion-se-captura-para-poder-agrupar.md) | País y estado cerrados, ciudad sugerida y normalizada; `state_to` deja de guardar países (b098 los movió a `country_to`); el correo del proveedor es opcional pero la BD exige al menos un contacto | aceptada |
| [0058](0058-el-calendario-de-huecos-es-una-regla-no-un-modelo.md) | El calendario de huecos es `temporadas fijas − salidas` (función pura en git, alcances `nacional`/`frontera`, `duration_days` y `meses_ideales` en `services`), lista en `/salidas` para admins y agentes, aviso semanal del Clawbot; la IA solo redacta al clic; nunca crea salidas; cada sugerencia y su destino se guardan desde el día uno para poder medir en 2027 (b099, construido el mismo día) | aceptada |
